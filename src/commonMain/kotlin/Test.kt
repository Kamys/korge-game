...
private class BunnyContainer(maxSize: Int) : FSprites(maxSize) {
    ...
    var FSprite.speedXf: Float
        get() = speeds[index * 2 + 0];
        set(value) {
            speeds[index * 2 + 0] = value
        }
    var FSprite.speedYf: Float
        get() = speeds[index * 2 + 1];
        set(value) {
            speeds[index * 2 + 1] = value
        }
...
suspend fun main() = Korge(width = 800, height = 600, bgcolor = Colors["#2b2b9b"]) {
    val wabbitTexture = resourcesVfs["korge.png"].readBitmap()
    ...
    val bunnyCountText = text("", font = font, textSize = 16.0, alignment = TextAlignment.TOP_LEFT).position(32.0, 32.0)
